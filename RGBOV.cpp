...
#define WHEEL_SPEED_TIMER_OCR1A			4
#define WHEEL_SPEED_TIMER_PRESCALER		256
#define WHEEL_SPEED_TIMER_PRESC_BITS	(1<<CS02)|(0<<CS01)|(0<<CS00)
#define WHEEL_SPEED_TIMER_FREQ_INT		(F_CPU/WHEEL_SPEED_TIMER_PRESCALER/WHEEL_SPEED_TIMER_OCR1A)
#define WHEEL_SPEED_TIMER_FREQ_FLOAT	((float)F_CPU/(float)WHEEL_SPEED_TIMER_PRESCALER/(float)WHEEL_SPEED_TIMER_OCR1A)
...
void updateWheelSpeed() {
	...
	if (nNewWheelTick - (nLastWheelTick > 100)	{		// This is a dumb-ass debouncing circuit
		nTicksPerRevolution = nNewWheelTick - nLastWheelTick;
		nLastWheelTick = nTicksPerRevolution;
	}
}

void updateLEDtimerToWheelSpeed() {
...
ISR(INT0_vect) {
	...
	// We want the intensity timer tick to occur INTENSITY_LEVELS times per horizontal pixel
	OCR1A = round(INTENS_TIMER_BASEFREQ_FLOAT/fWheelFreq/(float)INTENSITY_LEVELS/(float)HORZ_RESOLUTION);		// We 
	
	// 3. Reset the pixel position
	idxHorizontalPixel = idxIntensity = 0;
	
}

ISR(TIMER1_COMPA_vect ) {
	// Timebase for intensity
	
	static uint8_t nColumnData[COLUMN_DATA_BYTES];
	
	if (idxHorizontalPixel>=HORZ_RESOLUTION) {
		// Reached the end of all rows, send all zeros
		...
		for (int idxRow=0; idxRow<VERTICAL_PIXELS; idxRow++) {
			for (int idxColor = 6; idxColor>=0; idxColor-=2) {	/* start with red (00110000) */
				uint8_t nColorIntensity = (0x03<<idxColor) & graphic[idxRow][idxHorizontalPixel] >> idxColor;
				if ((nColorIntensity>=idxIntensity)) {
		...
		if (++idxIntensity >= INTENSITY_LEVELS) {
			idxHorizontalPixel++;
		}
...
int main() {
	...
	TCCR0B = 0
		...
		;
	OCR0A = WHEEL_SPEED_TIMER_OCR1A;	// Timer 0 frequency = (8Mhz/256*4  = 7812.5Hz
